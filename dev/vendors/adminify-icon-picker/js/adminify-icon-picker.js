(function ($)
{
	...
	AI_Icon_Picker.prototype.sideBarListMarkup = function ()
	{
		...
		}).forEach(function (item, index)
		{
			...
			{
				markup += '<div class="aim-modal--sidebar-tab-item" data-library-id="' + item['library-id'] + '">' + item['title'] + '</div>';
			} else
			{
				markup += '<div class="aim-modal--sidebar-tab-item aesthetic-active" data-library-id="' + item['library-id'] + '">' + item['title'] + '</div>';
			}
		});
	...
	AI_Icon_Picker.prototype.setSideBarList = function (library, object)
	{
		object.forEach(function (item, index)
		{
			var icon_title = (item[0] !== '') ? library + ' - ' + item[0] : library,
				listItem = {
					"title": icon_title,
					"list-icon": item[1]['list-icon'].length ? item[1]['list-icon'] : "",
	...
	AI_Icon_Picker.prototype.customIconSidebarMenu = function ()
	{
		...
		var markup = '<div class="aim-modal--sidebar-tab-item" data-library-id="custom-icon">Custom Icons</div>';
